...
All Classifier instances that make up the default binary catalog.
Separated from the loader (linux_binary.py) so the data can be read and
extended without touching the mechanism — same pattern as win_registry_data.py
and win_binary_data.py.

Order is significant (drives precedence), matching Syft's DefaultClassifiers
sequence. Glance additions are appended after the Syft table.
"""
...
_python_version_template = rb"(?m)\x00(?P<version>{{ .version }}[-._a-zA-Z0-9]*)\x00"

_libpython_matcher = filename_template(
    r"(?:.*/|^)libpython(?P<version>[0-9]+(?:\.[0-9]+)+)[a-z]?\.so.*$",
    _python_version_template,
)

# ruby 3.4.0dev (2024-09-15T01:06:11Z master 532af89e3b) [x86_64-linux]
# ruby 2.7.7p221 (2022-11-24 revision 168ec2b1e5) [x86_64-linux]
_ruby_matcher = contents(
    rb"(?m)ruby (?P<version>[0-9]+\.[0-9]+\.[0-9]+((p|preview|rc|dev)[0-9]*)?) "
)

# ---------------------------------------------------------------------------
# Main classifier table (Syft binary table + glance additions)
# ---------------------------------------------------------------------------

LINUX_BINARY_CLASSIFIERS: list[Classifier] = [
    Classifier(
        cls="python-binary",
        file_globs=["**/python*"],
        matcher=any_of(
    ...
    Classifier(
        cls="perl-binary",
        file_globs=["**/perl"],
        matcher=contents(rb"(?m)\/usr\/local\/lib\/perl\d\/(?P<version>[0-9]+\.[0-9]+\.[0-9]+)"),
        package="perl",
    ...
    Classifier(
        ...
        matcher=any_of(
            _ruby_matcher,
            shared_library(r"^libruby\.so.*$", _ruby_matcher),
        ),
        package="ruby",
    ...
    Classifier(
        ...
        matcher=any_of(
            ...
            contents(rb"\x00+(?P<version>[0-9]+\.[0-9]+(\.[0-9]+){0,2}(-rc[0-9])?)\x00+Erlang/OTP"),
    ...
    Classifier(
        ...
        matcher=any_of(
            ...
            contents(rb"(?s)\x00(?P<version>1\.[12][0-9]\.[0-9]+(-dev)?)\x00.{0,1000}Unable to"),
            # [NUL]1.22.11[NUL]...ValidationError
            contents(rb"(?s)\x00(?P<version>1\.2[0-9]\.[0-9]+(-dev)?)\x00.{0,580}ValidationError"),
            contents(rb"(?s)\x00(?P<version>1\.1[0-9]\.[0-9]+(-dev)?)\x00.{0,1000}ValidationError"),
    ...
    Classifier(
        ...
        matcher=branching(
            ...
            Classifier(
                ...
                cpe_templates=["cpe:2.3:a:oracle:openjdk:{version}:update{{.update}}:*:*:*:*:*:*"],
            ),
            Classifier(
